...
if __name__ == "__main__":
    ctx = mx.cpu()
    num_of_vertices = 307
    A, indices_dict = utils.get_adj_matrix(
    ...
    cheb_polys = [nd.array(i, ctx=ctx)
                  for i in utils.cheb_polynomial(L_tilde, 3)]
    backbones = [
        {
    ...
    x = nd.random_uniform(shape=(16, 1, num_of_vertices, 12), ctx=ctx)
    print(net(x).shape)
    print(utils.predict(net, x, 12, 12).shape)
